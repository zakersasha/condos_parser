...
from onthemarket.config import base_id, units_table, general_table, AIR_TABLE_HEADERS
...
def get_old_units_data(existing_data, units):
    ...
    list_1_key_set = {(d['unit_type'], d['floor_plan_image_links']) for d in old_data}
    for dict_2 in units:
        if (dict_2['unit_type'], dict_2['floor_plan_image_links']) not in list_1_key_set:
